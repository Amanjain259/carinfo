...
function ProductList() {
  ...
  return (
    <div>
      <h1 className=' m-4  pt-2 font-bold text-3xl text-center text-black-700'>Cars</h1>
      <div className='font-bold p-8 text-xl text-center'>
        <label className=' m-8 text-black-700'>
          Category:
          <select className='border w-1/6 h-10 rounded-lg m-2 hover:bg-slate-200' value={category} onChange={handleCategoryChange}>
            <option value="All">All</option>
            <option value="Category 1">BMW</option>
            <option value="Category 2">Ferarrai</option>
            <option value="Category 3">Honda</option>
            <option value="Category 4">Audi</option>
          </select>
        </label>
        <label className=' m-8 text-black-700'>
          Min Price:
        ...
        {filteredProducts.map((product) => (
          <li key={product.name}>
            ...
            <div className='m-2 ml-16 w-96 mb-8'>
              <h3 className='font-bold text-lg text-black-700'>{product.name}</h3>
